...
export default async function MangaDetailPage({ params }: PageProps) {
  ...
  const related = media.relations.edges.filter(
      ...
      <div className="-mt-16 relative z-10">
        ...
        <div className="md:hidden">
          <div className="flex gap-4 mb-6">
            <div className="relative shrink-0 rounded-lg overflow-hidden shadow-2xl" style={{ width: 120, height: 170 }}>
              {media.coverImage.extraLarge && (
                <Image src={media.coverImage.extraLarge} alt={title} fill sizes="120px" className="object-cover" />
              )}
            </div>
          ...
          {media.title.native && title !== media.title.native && (
            <p className="text-sm mb-4" style={{ color: "var(--fg-subtle)" }}>{media.title.native}</p>
          )}

          <div className="flex flex-col gap-3 mb-6">
            <MangaWatchlistButton animeId={numId} initialStatus={userWatchlistStatus} isLoggedIn={!!session} />
          ...
          {media.genres.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {media.genres.map((g) => (
                <Link key={g} href={`/search?type=MANGA&genre=${encodeURIComponent(g)}`} className="badge">
                  {g}
                </Link>
              ))}
            </div>
          ...
          <MangaMetadataGrid media={media} />

          {media.description && (
            <div className="mb-6">
              <h2 className="text-sm font-semibold mb-2" style={{ color: "var(--fg-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Synopsis
              </h2>
        ...
        <div className="hidden md:flex gap-8">
          <div className="flex-1 min-w-0">
            ...
            {media.title.native && title !== media.title.native && (
              <p className="text-base mb-5" style={{ color: "var(--fg-subtle)" }}>{media.title.native}</p>
            )}
            ...
            {media.description && (
              <div className="mb-6">
                <h2 className="text-xs font-semibold mb-2" style={{ color: "var(--fg-subtle)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                  Synopsis
                </h2>
                <DescriptionToggle description={media.description} />
              </div>
          ...
          <div className="w-72 shrink-0 flex flex-col gap-5 pt-4">
            <div className="relative rounded-xl overflow-hidden shadow-2xl self-start" style={{ width: 200, height: 285 }}>
              {media.coverImage.extraLarge && (
                <Image src={media.coverImage.extraLarge} alt={title} fill sizes="200px" className="object-cover" />
              )}
            </div>
            {media.averageScore !== null && (
              <span
            ...
            <MangaWatchlistButton animeId={numId} initialStatus={userWatchlistStatus} isLoggedIn={!!session} />
            <RatingInput animeId={numId} initialRating={userRating} isLoggedIn={!!session} />
            <MangaMetadataGrid media={media} compact />
          </div>
        </div>
              ...
              {media.characters.edges.map(({ node, role }) => (
                <div key={node.id} className="flex flex-col items-center gap-1">
                  <div className="relative rounded-full overflow-hidden" style={{ width: 64, height: 64 }}>
                    {node.image.medium ? (
                      <Image src={node.image.medium} alt={node.name.full ?? ""} fill sizes="64px" className="object-cover" />
                    ) : (
                      <div className="w-full h-full" style={{ background: "var(--bg-card)" }} />
                    )}
                  </div>
...
function MangaMetadataGrid({
  ...
  const rows = [
    ...
    { label: "Status", value: media.status ? formatStatus(media.status) : null },
    { label: "Chapters", value: media.chapters?.toString() ?? null },
    { label: "Volumes", value: media.volumes?.toString() ?? null },
    { label: "Author", value: studios[0]?.name ?? null },
    { label: "Popularity", value: media.popularity ? `#${media.popularity.toLocaleString()}` : null },
  ].filter((r) => r.value);

  return (
    <div className={`grid gap-x-4 gap-y-2 mb-6 ${compact ? "grid-cols-1" : "grid-cols-2"}`}>
