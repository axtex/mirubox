...
export interface TrackerSidebarBlockProps {
  mediaId: number;
  ...
  /** Mobile: status + heart + progress only (no rating/review). */
  compact?: boolean;
}
...
/** Tracked box height when dropdown is open (padding + border included). Desktop full block only. */
...
export function TrackerSidebarBlock({
  ...
  initialReview,
  compact = false,
}: TrackerSidebarBlockProps): React.JSX.Element {
  ...
  const showRatingReview = !compact && isLoggedIn && isTracked && status && !pickerOpen;
      ...
      <div>
        ...
        <div
          style={{
            ...
            ...(!compact && isTracked ? { minHeight: TRACKED_BOX_MIN_HEIGHT } : {}),
...
/** Mobile-only rating + review block (shown when title is tracked). */
export function DetailRatingReviewBlock({
  mediaId,
  title,
  initialRating,
  initialReview,
  ...
  if (!isLoggedIn || !isTracked) return null;
  ...
  return (
    <div>
      <p style={SECTION_TITLE}>YOUR RATING</p>
      <div style={BLOCK_STYLE}>
        <div className="flex gap-1" onMouseLeave={() => setHoverRating(null)}>
          {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
            <button
